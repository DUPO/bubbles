...
const CMYK_OFFSET_DIST = 0.45;
const CMYK_DOT_SCALE = 0.55;
const CMYK_ALPHA = 180;
const JITTER_AMOUNT = 1.8;
const JITTER_CHANNEL = 0.6;
...
function draw() {
  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    // ── 6. Draw CMYK cluster with jitter ──
    if (p.r > 0.3) {
      const jx = p.x + (Math.random() - 0.5) * JITTER_AMOUNT;
      const jy = p.y + (Math.random() - 0.5) * JITTER_AMOUNT;
      ...
      for (let ch = 0; ch < CMYK_CHANNELS.length; ch++) {
        ...
        if (dotR > 0.2) {
          ...
          circle(
            jx + chan.cos * offsetR + (Math.random() - 0.5) * JITTER_CHANNEL,
            jy + chan.sin * offsetR + (Math.random() - 0.5) * JITTER_CHANNEL,
            dotR * 2
          );
        }
      }
