...
function rebuildGrid() {
  ...
  for (let c = 0; c < cols; c++) {
    ...
    for (let r = 0; r < rows; r++) {
      ...
      // Continuous value from distance + noise
      const sizeFactor = Math.pow(1 - trailT, FALLOFF_CURVE);
      const nLow = noise(c * 0.1, r * 0.1);
      const nHigh = noise(c * 0.5 + 100, r * 0.5 + 100);
      const noiseShift = (nLow - 0.5) * SIZE_NOISE + (nHigh - 0.5) * SIZE_NOISE * 0.6;

      // Map to 0–1 then quantize into exactly 13 steps
      let t = constrain(sizeFactor + noiseShift * 0.5, 0, 1);
      const step = floor(t * SIZE_STEPS);
      const clampedStep = constrain(step, 0, SIZE_STEPS - 1);

      // Each step is a distinct radius — exponential spacing so small sizes
      // are visually distinguishable from each other (not all clustered near 0)
      const stepT = clampedStep / (SIZE_STEPS - 1);
      const expT = Math.pow(stepT, 1.8); // exponential: more steps in the small range
      let baseR = minR + (maxR - minR) * expT;
